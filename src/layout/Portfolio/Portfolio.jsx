...
import {
    faHtml5, faCss3Alt, faReact, faSass, faJsSquare, faWordpress
...
const cardsData = [
    ...
    },
    {
        ...
        image: PixPicture,
        title: "Pix&Motion",
        description: "Description du projet Pix&Motion.",
    ...
    },
    {
        ...
        description: "Description du projet Pix&Motion Tournament.",
    ...
    },
    {
        ...
        description: "Description du projet LufyDoom Coaching.",
    ...
    return (
        <section id="myportfolio" className="portfolio">
            <div className="banner-container">
                <img className="banner" src={bannerImage} alt="Bannière du portfolio de projets" />
                <h1 className="banner-title">Portfolio</h1>
                <p className="banner-text">
                    Découvrez divers projets réalisés au cours de ma formation ainsi que d'autres initiatives professionnelles.
                </p>
            </div>
            ...
            <div className="card-container">
                {cardsData.map((card) => (
                    <article className="card1" key={card.id}>
                        <img src={card.image} alt={`Capture d'écran de ${card.title}`} className="card-image" />
                        <h3 className="card-title">{card.title}</h3>
                        <div className="card-icons">
                            {card.icons.map((icon, index) => (
                                <FontAwesomeIcon key={index} icon={icon} aria-hidden="true" />
                            ))}
                        </div>
                        <button
            ...
            {showModal && (
                <div className="modal-overlay" onClick={closeModal}>
                    <div className="modal-content" onClick={(e) => e.stopPropagation()}>
                        <button className="close-modal" onClick={closeModal} aria-label="Fermer la fenêtre">
                            <FontAwesomeIcon icon={faTimes} />
                        </button>
                        <img src={modalContent.image} alt={`Capture d'écran de ${modalContent.title}`} className="modal-image" />
                        <h2>{modalContent.title}</h2>
                        <p>{modalContent.description}</p>
                        <a href={modalContent.url} target="_blank" rel="noopener noreferrer" aria-label={`Voir le projet ${modalContent.title}`}>
